...
class MongoDB:
    ...
    def insert_document(self,account,file_document,document_type):
        db = self.client['Accounts']
        collection = db['accounts_details']
        insert_done = collection.update_one({'_id':account['_id']},{'$push': {'uploaded_documents': file_document}})
        db1 = self.client['AccountHolderDocuments']
        collection1 = db1['accounts']
        print(account)
        insert_done1 = collection1.update_one({'_id':account['_id']},{'$push': {'uploaded_documents': file_document}},upsert=True)
